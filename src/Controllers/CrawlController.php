...
class CrawlController extends CrudController
{
        ...
        try {
            ...
            foreach ($request['link'] as $link) {
                if (preg_match('/(.*?)(\/phim\/)(.*?)/', $link)) {
                    $link = sprintf('%s/phim/%s', config('ophim_crawler.domain', 'https://apii.online/kkphim'), explode('phim/', $link)[1]);
    ...
    public function showCrawlPage(Request $request)
    {
        ...
        try {
            $categories = Cache::remember('ophim_categories', 86400, function () {
                $data = json_decode(file_get_contents(sprintf('%s/the-loai', config('ophim_crawler.domain', 'https://apii.online/kkphim'))), true) ?? [];
                return collect($data)->pluck('name', 'name')->toArray();
            });

            $regions = Cache::remember('ophim_regions', 86400, function () {
                $data = json_decode(file_get_contents(sprintf('%s/quoc-gia', config('ophim_crawler.domain', 'https://apii.online/kkphim'))), true) ?? [];
                return collect($data)->pluck('name', 'name')->toArray();
            });
        } catch (\Throwable $th) {
    ...
    {
        $pattern = sprintf('%s/phim/{slug}', config('ophim_crawler.domain', 'https://apii.online/kkphim'));
        try {
